...
//        }
    ...
    else
    {
        ...
        if (serialPort->bytesAvailable() > 0)
        {
            QByteArray rxdata = serialPort->readAll();  // Read all available data
            //        qDebug() << "Received Data: "<<data;
            Application.RxData = rxdata;
            ...
            SerialRxData = hexToString(rxdata);  // Convert the received data from hex to string
        } else {
            SerialRxData = "";
...
void SerialPortWidget::onSendDataClicked()
{
    if(Application.isModbusSerialEnabled)
    {
        QMessageBox::warning(this, "send error", "Disable Modbus over serial to send data from here." );
        return;
    }
    if(!serialPort->isOpen())
    {
