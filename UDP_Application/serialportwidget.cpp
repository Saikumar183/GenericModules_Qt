...
//    comboBoxBaudRate->addItem("1000000", QSerialPort::Baud1000000);
    ...
    QTimer *timer_100 = new QTimer(this);
    timer_100->setInterval(50);
    this->timer_100 = timer_100;
    this->count = 0;
...
//    labelStopBits->setStyleSheet("QLabel { border: none; font-weight: bold; color: #1B263B; }");
    ...
    mainLayout->addWidget(pushButtonSend);
...
void SerialPortWidget::readSerialPortSettingsAndData()
{
    ...
    if (serialPort->bytesAvailable() > 0) {
        ...
        QString receivedData = hexToString(data);  // Convert the received data from hex to string
        showdata(receivedData);  // Emit signal with the received data
    } else {
...
void SerialPortWidget::onSendDataClicked() {
    ...
    if (!data.isEmpty() && serialPort->isOpen())  // Check if input data is not empty and serial port is open
    {
        ...
        if (count < 10) {
            // Here we simulate the action of the pushButtonSend_100 clicked
            serialPort->write(sendData);
            // Replace this line with your actual updatePortList implementation
            //            qDebug() << "Sending command #" << count + 1;
